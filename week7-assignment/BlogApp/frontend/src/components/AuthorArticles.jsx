  ...
  useEffect(() => {
    ...
    const getAuthorArticles = async () => {
      try {
        ...
        let res = await axios.get(" http://localhost:4002/author-api/articles", { withCredentials: true });
        if (res.status === 200) {
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
      {articles.map((article) => (
        <div key={article._id} className={`${articleCardClass} relative flex flex-col`}>
          {/* Status Badge */}
          <span className={article.isArcticleActive ? articleStatusActive : articleStatusDeleted}>
            {article.isArcticleActive ? "ACTIVE" : "DELETED"}
          </span>
