...
function ArticleByID() {
  ...
  useEffect(() => {
    ...
    const getArticle = async () => {
      ...
      try {
        const res = await axios.get(`http://localhost:4002/user-api/article/${id}`, { withCredentials: true });
  ...
  const toggleArticleStatus = async () => {
    const newStatus = !article.isArcticleActive;
    ...
    try {
      const res = await axios.patch(
        "http://localhost:4002/author-api/articles",
        { articleId: article._id, isArcticleActive: newStatus },
      ...
      {user?.role === "AUTHOR" && (
        <div className={articleActions}>
          ...
          <button className={deleteBtn} onClick={toggleArticleStatus}>
            {article.isArcticleActive ? "Delete" : "Restore"}
          </button>
        </div>
      )}
